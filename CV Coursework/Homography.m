...
% Compute epipoles
epi1 = null(F'); % Right epipole
epi2 = null(F);  % Left epipole
epi1 = epi1 / epi1(3); % Normalize
epi2 = epi2 / epi2(3);

disp('Epipole in Image 1:');
disp(epi1');
disp('Epipole in Image 2:');
disp(epi2');
...
subplot(1,2,1);
imshow(img1); hold on;
title('Epipolar Lines, Epipole & Keypoints in Image 1');
epiLines1 = epipolarLine(F', matchedPoints2.Location);
points = lineToBorderPoints(epiLines1, size(img1));
line(points(:, [1,3])', points(:, [2,4])', 'Color', 'r');
plot(matchedPoints1.Location(:,1), matchedPoints1.Location(:,2), 'go');
plot(epi1(1), epi1(2), 'rx', 'MarkerSize', 15, 'LineWidth', 2);

subplot(1,2,2);
imshow(img2); hold on;
title('Epipolar Lines, Epipole & Keypoints in Image 2');
epiLines2 = epipolarLine(F, matchedPoints1.Location);
points = lineToBorderPoints(epiLines2, size(img2));
line(points(:, [1,3])', points(:, [2,4])', 'Color', 'b');
plot(matchedPoints2.Location(:,1), matchedPoints2.Location(:,2), 'go');
plot(epi2(1), epi2(2), 'rx', 'MarkerSize', 15, 'LineWidth', 2);
